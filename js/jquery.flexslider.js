...
.attr('tabindex', '-1');
    ...
    slider.setup = function(type) {
      ...
      if (!fade) {
        ...
        if (type === 'init') {
          slider.viewport = $('<div class="' + namespace + 'viewport"></div>').css({overflow: 'hidden', position: 'relative'}).appendTo(slider)
...
.attr('aria-hidden', 'true')).prepend(slider.slides.last().clone().addClass('clone')
.attr('aria-hidden', 'true'));
        ...
        if (type === 'init') {
          if (!touch) {
            ...
            slider.slides.css({ opacity: 0, display: 'block', 'zIndex': 1 }).eq(slider.currentSlide).css({'zIndex': 2})
